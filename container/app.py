from fastai_segmentation import segmenter
from detectron_object_detector import detector
import io
...
# run inference
def run_inference(img_path = 'file.jpg', low_memory = False):

	if not low_memory:
		result_img = model.inference(img_path)
		...
		# run inference
		untransformed_result = model.inference(transformed_path)
...
def upload():
	...
	# run inference
	# result_img = run_inference(utils._convert_pillow_to_cv2(rgb_im))
